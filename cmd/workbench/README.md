...
文字会话按 profile 和协议隔离，在当前浏览器标签页记住会话和续接点。页面只展示当前分支，每次请求只使用选择父节点的祖先路径；「从这里继续」创建分支，「复制为新会话」保留该路径及归属。会话列表的「管理」支持重命名和删除。更换实际模型时保留当前 profile 的用户附件，旧模型的助手工具状态转成普通文本。完整的原生输出和流式事件保存在会话中，可展开检查。
...
上传和创建使用弹窗，可先预览请求。删除与取消的确认弹窗会等待请求结束，失败后保留上下文供重试。分页失败保留当前列表；文件及 JSONL 结果下载保留原始字节。页面不自动翻页、重复提交或清理云端资源。会话删除只影响本地存储。
...
日志页支持按 profile 和操作筛选，在详情弹窗中查看时间、状态、request/response metadata 和正文；单个正文预览最多 256 KiB，截断时可下载完整记录。认证头和签名 URL 元信息脱敏，正文原样保留，包含提示词和上传文件。HTTP EOF 标志不表示模型完成；操作状态和会话错误应一并查看。日志写入失败会阻止请求或返回失败，不能据此认为服务端未执行并直接重试。
