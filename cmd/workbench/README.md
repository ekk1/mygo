...
历史正文直接展开阅读，仅原生结果可以折叠。历史与输入区放在同一个紧凑面板中，长对话在历史区统一滚动，输入区保持可用；手动向上阅读时，流式更新不把视图拉回底部。
...
浏览器覆盖五个入口的 37 个功能页在桌面和手机下的布局，以及模型选择、profile 冲突恢复、多轮旧会话恢复、会话分支、流式失败、资源分页、下载原文、弹窗确认和日志详情。历史正文与原生结果在四种主题下检查显示和键盘操作。临时服务和浏览器由测试清理，脚本错误与清理失败会使测试失败；截图写入已忽略的 `bin/workbench-browser/`。
