...
(() => {
 ...
 W.resources=async()=>{
  ...
  const status=el("div",{}),tableBox=el("div",{class:"table-scroll"}),pager=el("div",{class:"pagination"}),editor=el("div",{}),detail=el("section",{class:"resource-detail"});
  let rows=[],cursor="",next="",previous=[],loading=false;
  const listParams=()=>{const params=vendor==="gemini"?{pageSize:50}:{limit:50};if(cid)params.container_id=cid;if(cursor){if(vendor==="anthropic"&&feature==="files")params.after_id=cursor;else if(vendor==="gemini")params.pageToken=cursor;else if(vendor==="xai")params.pagination_token=cursor;else if(vendor==="anthropic")params.after_id=cursor;else params.after=cursor;}return params;};
  ...
  function rowActions(row){const actions=[button("详情",()=>show(row),"quiet")];
   ...
   if(feature==="batches"){
     ...
     const phase=String(row.status||row.processing_status||row.state||"").toLowerCase();if(!/(completed|ended|cancelled|canceled|failed|expired|succeeded)/.test(phase))actions.push(rowButton("取消任务",()=>W.confirm("取消这个 Batch 任务？",async()=>{await call("batches.cancel",withID(row));await load();}),"danger"));
  ...
  function render(){const query=search.value.trim().toLowerCase();const shown=rows.filter(row=>[row.filename,row.displayName,row.name,row.id,row.path].some(value=>String(value||"").toLowerCase().includes(query)));
    ...
    const prev=button("上一页",()=>{cursor=previous.pop()||"";load();});prev.disabled=!previous.length||loading;
    const nextButton=button("下一页",()=>{previous.push(cursor);cursor=next;load();});nextButton.disabled=!next||loading;
    pager.replaceChildren(el("span",{class:"muted small"},`本页 ${shown.length} 项 · 搜索仅筛选当前页`),el("div",{class:"actions"},prev,nextButton));
  }
  async function xaiResults(row,token="") {
    ...
    detail.replaceChildren(el("section",{class:"card stack"},el("h2",{},"Batch 结果（当前页）"),el("pre",{},pretty(data)),W.download(new Blob([pretty(data)],{type:"application/json"}),"batch-results-page.json"),nextToken?rowButton("下一页结果",()=>xaiResults(row,nextToken)):null));
  }
  function appendBatch(row) {
    const text=el("textarea",{rows:10},'[\n  {"batch_request_id":"request-1","batch_request":{"responses":{"model":"","input":[{"role":"user","content":"你好"}]}}}\n]');
    const output=el("div",{});let busy=false;
    async function send(preview){if(busy)return;busy=true;try{const body={batch_id:idOf(row),batch_requests:JSON.parse(text.value)};const result=await W.native("batches.requests",body,{},preview);output.replaceChildren(preview?W.preview(result):W.notice("请求已添加。"));if(!preview)await load();}catch(error){W.fail(error,output);}finally{busy=false;}}
    text.addEventListener("input",()=>output.replaceChildren());
    editor.replaceChildren(el("section",{class:"card stack"},el("h2",{},"向 Batch 添加请求"),field("原生 batch_requests 数组",text),el("div",{class:"actions"},button("预览请求",()=>send(true)),button("添加请求",()=>send(false))),output));
  }
  const refresh=button("刷新",()=>{cursor="";previous=[];load();});search.addEventListener("input",render);
  const primary=button(feature==="batches"?"创建 Batch":feature==="containers"&&!cid?"创建容器":"上传文件",()=>openEditor(),"");
  W.root.replaceChildren(W.heading(title,`${W.vendors[vendor].name} / ${profile.name}${cid?" / "+cid:""}`,el("div",{class:"actions"},cid?el("a",{href:W.url("containers"),class:"button quiet"},"返回容器"):null,cid?button("引用已有文件",()=>openEditor(true)):null,primary)),el("section",{class:"card resource-manager"},el("div",{class:"resource-toolbar"},field("搜索当前页",search),refresh),status,tableBox,pager),editor,detail);
  async function openEditor(reference=false){
   const fields=[],uploads={};let build;
   ...
   const form=el("form",{class:"card stack","data-resource-editor":""},el("div",{class:"section-head"},el("h2",{},reference?"引用已有文件":primary.textContent),button("关闭",()=>editor.replaceChildren(),"quiet")),fields,el("details",{},el("summary",{},"原生扩展参数"),field("额外 JSON 字段",extra)),el("div",{class:"actions"},submit,preview),out);
   let busy=false;async function perform(isPreview){if(busy||!form.reportValidity())return;busy=true;submit.disabled=true;preview.disabled=true;try{const req=build();const extras=JSON.parse(extra.value||"{}");if(!extras||Array.isArray(extras)||typeof extras!=="object")throw new Error("扩展参数必须是 JSON 对象");for(const [key,value]of Object.entries(extras)){if(Object.hasOwn(req.params,key))throw new Error(`重复字段 ${key}`);req.params[key]=value;}const data=await W.native(req.operation,req.params,req.uploads,isPreview);out.replaceChildren(isPreview?W.preview(data):W.notice("操作已完成。"));if(!isPreview)await load();}catch(error){out.replaceChildren(W.notice(error.message,true));}finally{busy=false;submit.disabled=false;preview.disabled=false;}}
   form.addEventListener("submit",event=>{event.preventDefault();perform(false);});form.addEventListener("input",()=>{if(out.childNodes.length)out.replaceChildren(W.notice("输入已变化，请重新预览。"));});editor.replaceChildren(form);form.scrollIntoView({block:"nearest"});
  }
  await load();
 };
})();
