...
(() => {
  ...
  function renderNavigation(){
    ...
    nav.append(el("div",{class:"nav-caption"},"工作区"),...[ ["media","↓","媒体下载器"],["library","▧","资产库"],["tasks","◷","后台任务"],["usage","◎","Token 用量"] ].map(([id,mark,label])=>el("a",{href:"/"+id,class:"nav-link",...(parts[0]===id?{"aria-current":"page"}:{})},mark,el("span",{},label))));
  ...
  function home(){W.root.replaceChildren(W.heading(parts[0]==="ai"?"AI":"个人工作台",parts[0]==="ai"?"选择服务商，进入自己的工作空间。":"一个入口，处理日常工作。"));if(parts[0]!=="ai"){W.root.append(el("a",{href:"/media",class:"module-entry"},el("span",{class:"module-symbol"},"↓"),el("div",{},el("h2",{},"媒体下载器"),el("p",{},"下载播客和视频，抽取音频并保存到资产库")),el("span",{},"进入 →")));W.root.append(el("a",{href:"/ai",class:"module-entry"},el("span",{class:"module-symbol"},"✦"),el("div",{},el("h2",{},"AI"),el("p",{},"文字、图片、音频、视频与云端资源")),el("span",{},"进入 →")));return;}
  ...
  function profileSettings(){
    ...
    W.root.append(el("div",{class:"profile-list"},profiles.map(p=>el("section",{class:"card profile-row"},el("div",{},el("h2",{},p.name),el("p",{class:"muted"},p.base_url),el("span",{class:"badge"},p.has_key?"已保存 key":"未设置 key")),el("div",{class:"actions"},el("a",{href:W.url("chat",p.id),class:"button"},"进入"),el("a",{href:"/usage?profile_id="+encodeURIComponent(p.id),class:"button quiet"},"Token 用量"),button("编辑",()=>editProfile(p)),button("复制",()=>editProfile({...p,id:null,name:p.name+" 副本",has_key:false,api_key:""})),button("删除",()=>W.confirm(`删除 profile「${p.name}」？`,async()=>{const before=W.config.providers;W.config.providers=before.filter(item=>item.id!==p.id);try{await saveConfig();}catch(error){W.config.providers=before;throw error;}profileSettings();},"删除本地连接配置；不会删除服务商的云端资源。"),"danger"))))));
  ...
  async function boot(){try{W.config=await W.api("/api/config");if(vendor&&!vendors[vendor])throw new Error("服务商入口不存在");if(profileID){W.profile=W.config.providers.find(p=>p.id===profileID&&p.kind===vendor);if(!W.profile)throw new Error("此服务商下找不到这个 profile，请重新选择。");sessionStorage.setItem(`wb-profile-${vendor}`,profileID);}else if(vendor&&feature!=="profiles"){const list=W.config.providers.filter(p=>p.kind===vendor);if(list.length){const remembered=sessionStorage.getItem(`wb-profile-${vendor}`);location.replace(W.url("chat",list.find(p=>p.id===remembered)?.id||list[0].id));return;}}
      ...
      document.title=[W.profile?W.menu().find(([id])=>id===feature)?.[1]:null,W.profile?.name,vendors[vendor]?.name||({settings:"全局设置",logs:"请求日志",ai:"AI",media:"媒体下载器",usage:"Token 用量",library:"资产库",tasks:"后台任务"})[parts[0]],"个人工作台"].filter(Boolean).join(" · ");
      renderNavigation();if(parts[0]==="settings")await settings();else if(parts[0]==="logs")await logs();else if(parts[0]==="usage")await W.usage();else if(parts[0]==="media")await W.media();else if(parts[0]==="library")await W.library();else if(parts[0]==="tasks")await W.tasks();else if(vendor&&(feature==="profiles"||!W.profile))profileSettings();else if(W.profile){if(!W.menu().some(([id])=>id===feature))throw new Error("当前 profile 没有启用此功能。");if(["files","containers","batches"].includes(feature))await W.resources();else await W.workspace();}else home();
